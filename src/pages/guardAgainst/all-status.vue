<template>
  ...
  >
    <div slot="header" class="table-title">
      ...
      <Button 
        type="primary" 
        :disabled="multipleChoose"
        @click="resetTaskHandler"
        style="marginRight: 10px;">撤销</Button>
      <!-- <Button 
        type="primary" 
        :disabled="singleChoose"
        >提醒</Button> -->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getListData () {
        const _this = this
        axios.get('/patrolManage/findPatrolPage.action?patrol.organization.id='+this.currOrgManageId+'&page='+_this.currListPage+'&rows='+_this.listPageSize)
        // axios.get('./static/mock/wisdomEyes/allstatus.json')
          .then(res => {
            // console.log(res)
            _this.allStatusList = res.data.rows
