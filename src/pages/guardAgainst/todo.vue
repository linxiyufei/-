...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getListData () {
        const _this = this
        axios.get('/patrolManage/findPatrolPage.action?patrol.organization.id='+this.currOrgManageId+'&page='+_this.currListPage+'&rows='+_this.listPageSize+'&patrol.status=1')
        // axios.get('./static/mock/wisdomEyes/allstatus.json')
          .then(res => {
            // console.log(res)
            _this.allStatusList = res.data.rows
